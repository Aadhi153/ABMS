...
const HISTORY_QUERY = gql`
  query SalaryHistoryTabData {
    salaryRevisions(revisionType: "INCREMENT") {
      id
      employeeName
      employeeDepartment
      previousSalary
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-foreground">Salary Increment History Log</h2>
          <p className="text-sm text-muted-foreground">Auditable record of all monthly basic revisions and allowances changes.</p>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30 text-left text-muted-foreground">
                  <th className="px-4 py-2.5 font-medium">Increment Date</th>
                  <th className="px-4 py-2.5 font-medium">Employee</th>
                  <th className="px-4 py-2.5 font-medium">Department</th>
                  <th className="px-4 py-2.5 font-medium">Salary Revision</th>
                  <th className="px-4 py-2.5 font-medium text-right">Change Amount</th>
                ...
                {!loading && revisions.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-4 py-10 text-center text-muted-foreground">
                      No salary history logs found
                    </td>
                  </tr>
                )}
                {revisions.map((r, idx) => {
                  const change = r.newSalary - r.previousSalary;
                  return (
                    <tr
                      key={r.id}
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 text-muted-foreground">{r.employeeDepartment ?? "—"}</td>
                      <td className="px-4 py-2.5 text-muted-foreground">
                        {inr(r.previousSalary)} → <span className="font-medium text-foreground">{inr(r.newSalary)}</span>
                      </td>
                      <td className={cn("px-4 py-2.5 text-right font-medium", change >= 0 ? "text-success" : "text-danger")}>
                        {change >= 0 ? "+" : ""}
                        {inr(change)}
                      </td>
